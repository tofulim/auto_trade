...
class LLMTradingStrategy:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("GEMINI_API_KEY not set. LLM strategy will be disabled.")
            self.enabled = False
            return
    ...
    def analyze_stock_and_decide(
        ...
        try:
            ...
            logger.info(f"LLM analysis for {stock_symbol}: {decision} (confidence: {confidence})")
            return decision, reasoning, confidence
            
        except Exception as e:
            logger.error(f"Error in LLM analysis for {stock_symbol}: {e}")
    ...
    def _get_market_context(self, stock_symbol: str, period_days: int) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "current_vs_high": round(((latest_close - stock_data['High'].max()) / stock_data['High'].max()) * 100, 2),
