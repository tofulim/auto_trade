...
def check_order_exist(next_task_name: str, **kwargs):
    ...
    # 매매 신청한 종목들을 구한다.
    candidate_portfolio_rows = list(filter(lambda row: row["order_status"] in ["O", "W"], portfolio_rows))
    ...
    if len(candidate_portfolio_rows) > 0:
        ...
        logger.info("no order exist. run task_empty")
        return "task_empty"
...
def change_status(**kwargs):
    ...
    for rsvn_order in rsvn_orders:
        ...
        try:
            ...
            if prcs_rslt == "미처리":
                order_status = "F"
                update_dict.update({"order_status": order_status})
            # 정상 처리된 경우
            elif prcs_rslt == "처리":
                order_status = "S"
                update_dict.update(
            ...
            logger.info(f"종목 {pdno}을 처리하던 중 에러 발생 {e}")
            order_status = "E"
        finally:
