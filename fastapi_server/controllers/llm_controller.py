...
def analyze_stock_with_llm(request: LLMAnalysisRequest):
    ...
    """
    logger.info(f"LLM analysis requested for {request.stock_symbol}", extra={"endpoint_name": "analyze_stock_with_llm"})
    ...
    try:
        ...
        if not strategy.is_enabled():
            logger.warning("LLM strategy not available", extra={"endpoint_name": "analyze_stock_with_llm"})
            return LLMAnalysisResponse(
                decision="HOLD", reasoning="LLM strategy not available", confidence=0.0, enabled=False
            )
        ...
        return LLMAnalysisResponse(decision=decision, reasoning=reasoning, confidence=confidence, enabled=True)

    except Exception as e:
        logger.error(
            f"Error in LLM analysis for {request.stock_symbol}: {e}", extra={"endpoint_name": "analyze_stock_with_llm"}
        )
        return LLMAnalysisResponse(
            decision="HOLD", reasoning=f"LLM analysis failed: {str(e)}", confidence=0.0, enabled=False
        )
...
def check_llm_health():
    ...
    try:
        ...
        return {"status": "error", "enabled": False, "message": f"LLM health check failed: {str(e)}"}
